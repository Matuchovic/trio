...
function Scoop({ position, color, speed = 0.8, distort = 0.35, size = 1 }: any) {
  ...
  return (
    <Float speed={speed * 1.2} rotationIntensity={0.3} floatIntensity={0.9}>
...
function Ring({ r, color, speed }: any) {
  const ref = useRef<THREE.Mesh>(null);
  useFrame((s) => {
    if (!ref.current) return;
...
function Particles() {
  ...
  const cols = useMemo(() => {
    const a = new Float32Array(count * 3);
    const pal = [new THREE.Color("#C4752A"),new THREE.Color("#E8526A"),new THREE.Color("#4ABFA0"),new THREE.Color("#D4AF37")];
    for (let i = 0; i < count; i++) { const c = pal[i%4]; a[i*3]=c.r; a[i*3+1]=c.g; a[i*3+2]=c.b; }
    return a;
  }, []);
...
function Scene() {
  ...
  return (
    <group ref={ref}>
      <Scoop position={[-3.5,1.8,-1]} color="#5C3015" speed={0.7} distort={0.38} size={1.1} />
      <Scoop position={[3.2,2.2,-2.5]} color="#D4A843" speed={1.0} distort={0.3} size={1.0} />
      <Scoop position={[0.3,3.8,-3.5]} color="#E8526A" speed={0.6} distort={0.42} size={1.2} />
      <Scoop position={[-4.8,-1.2,-4]} color="#C4752A" speed={0.9} distort={0.28} size={0.8} />
      <Scoop position={[4.8,-0.8,-3]} color="#4ABFA0" speed={1.2} distort={0.32} size={0.85} />
      <Scoop position={[-1.5,-3,-5]} color="#D4AF37" speed={0.75} distort={0.25} size={0.7} />
      <Ring r={5.5} color="#C4752A" speed={0.06} />
      <Ring r={7.5} color="#E8526A" speed={-0.04} />
...
export default function HeroScene() {
  return (
    <Canvas camera={{ position:[0,0,9], fov:48 }} gl={{ antialias:true, alpha:true }} dpr={[1,2]}
      ...
      <pointLight position={[5,-4,2]} intensity={0.7} color="#E8526A" />
      <Scene />
      <Particles />
