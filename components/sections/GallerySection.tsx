    ...
    <section id="galerie" ref={ref} className="section" style={{ background:"var(--ink)" }}>
      <div className="wrap">
          ...
          <div>
            ...
            <div className="overflow-hidden">
              <motion.h2 className="t-display font-black italic"
                ...
                Každý záběr<br/><span className="grad-text">říká příběh</span>
              </motion.h2>
            </div>
          </div>
          <motion.p initial={{ opacity:0 }} animate={iv?{opacity:1}:{}} transition={{ delay:.3 }}
            className="t-body text-[15px] md:text-right" style={{ color:"rgba(251,245,235,.4)" }}>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 auto-rows-[200px]">
          {items.map((item,i)=>(
            <motion.div key={i}
              initial={{ opacity:0, scale:.92 }} animate={iv?{opacity:1,scale:1}:{}}
              transition={{ delay:i*.06, duration:.7, ease:[0.22,1,0.36,1] }}
              ...
              <svg className="absolute inset-0 w-full h-full opacity-5" viewBox="0 0 400 200">
                {Array.from({length:8}).map((_,j)=>(
                  <line key={j} x1={j*57} y1="0" x2={j*57} y2="200" stroke="white" strokeWidth="0.5"/>
                ))}
                {Array.from({length:5}).map((_,j)=>(
                  <line key={j} x1="0" y1={j*55} x2="400" y2={j*55} stroke="white" strokeWidth="0.5"/>
                ))}
              </svg>
              ...
              <motion.div className="absolute inset-0 flex flex-col justify-end p-5 opacity-0 group-hover:opacity-100 transition-opacity duration-400"
                ...
                <div className="t-label mb-1" style={{ color:"rgba(251,245,235,.5)", letterSpacing:".15em" }}>{item.sub}</div>
              ...
              <div className="absolute top-3 left-3 glass px-3 py-1 rounded-full"
                style={{ fontFamily:"var(--mono)", fontSize:"9px", letterSpacing:".2em", textTransform:"uppercase", color:"rgba(251,245,235,.5)" }}>
                {item.sub.split("·")[0].trim()}
              </div>

              {/* Expand icon */}
              <div className="absolute top-3 right-3 w-7 h-7 rounded-full glass flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
      ...
      <AnimatePresence>
        {sel!==null && (
          <motion.div initial={{ opacity:0 }} animate={{ opacity:1 }} exit={{ opacity:0 }}
            ...
            onClick={()=>setSel(null)}>
            <motion.div initial={{ scale:.88, opacity:0 }} animate={{ scale:1, opacity:1 }} exit={{ scale:.92, opacity:0 }}
              ...
              <svg className="absolute inset-0 w-full h-full opacity-[0.04]" viewBox="0 0 800 450">
                {Array.from({length:16}).map((_,j)=>(<line key={j} x1={j*55} y1="0" x2={j*55} y2="450" stroke="white" strokeWidth="0.5"/>))}
                {Array.from({length:9}).map((_,j)=>(<line key={j} x1="0" y1={j*55} x2="800" y2={j*55} stroke="white" strokeWidth="0.5"/>))}
              </svg>
              <div className="absolute bottom-8 left-8">
                <div className="t-label mb-2" style={{ color:"rgba(251,245,235,.4)", letterSpacing:".2em" }}>{items[sel].sub}</div>
                <div className="t-display text-4xl italic font-bold text-cream" style={{ fontFamily:"var(--display)" }}>{items[sel].label}</div>
              </div>
            </motion.div>
